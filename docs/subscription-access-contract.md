...
Series AV uses RevenueCat to perform App Store purchase and restore operations,
while Apps AV remains the authority for visible Pro access.
...
- An inactive purchase tells the user not to purchase again and points to
  Restore Purchases.
- An inactive restore reports that no active Series AV Pro subscription was
  found for the current App Store account and returns to the subscribe path.
...
operations, bounded retries, eventual Apps AV Pro, and account identity.
