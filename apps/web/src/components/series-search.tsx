...
export function SeriesSearch() {
  ...
  const search = useQuery({
    ...
    queryFn: () => client.searchSeries({ query: trimmedQuery, locale: "en-US", limit: 12 }),
    queryKey: ["series-av", "search", trimmedQuery]
  });
  ...
  return (
    <section className="flex flex-col gap-6">
      <div className="series-paper rounded-[1.5rem] border border-[#d7c494] p-5 shadow-lg shadow-[#172f5c]/8 sm:p-6">
        <div className="mb-5">
          <h1 className="text-3xl font-semibold text-[#112a55]">Search the Series AV catalog.</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[#53617a]">
            Find a title, review its artwork, and prepare it for the signed-in library workflow.
          </p>
        </div>
      ...
      {search.isError ? <ErrorState className="border-[#d7c494] bg-[#fff8df]" description={search.error.message} title="Series search failed" /> : null}
      {search.data && search.data.results.length === 0 ? <EmptyState className="border-[#d7c494] bg-[#fff8df]" description="Try a different title." title="No series found" /> : null}
            ...
            <Card key={result.id} className="gap-0 overflow-hidden rounded-[1.35rem] border-[#d7c494] bg-[#fff8df] py-0 shadow-sm shadow-[#172f5c]/8">
              <div className="aspect-[16/10] bg-[#ead6a5]">
                {result.posterUrl ? (
                  ...
                  <div className="flex h-full items-center justify-center text-sm font-medium text-[#748098]">No artwork</div>
                )}
              </div>
              <CardContent className="flex min-h-56 flex-col gap-3 p-4">
                <div>
                  ...
                  <p className="mt-2 flex items-center gap-2 text-xs font-medium uppercase tracking-[0.12em] text-[#5a8f2f]">
                    ...
                    {result.firstAirDate ?? "Date unknown"}
                  </p>
                </div>
                <p className="line-clamp-4 text-sm leading-6 text-[#53617a]">
                  {result.overview || "No overview is available yet."}
                </p>
              </CardContent>
            </Card>
          ))}
